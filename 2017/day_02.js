(() => {
    ...
    console.log('checksum 1:', day2_1(input));

    const day2_2 = input => {
        return input.trim().split('\n').reduce((result, value) => {
            return result += value.trim().split('\t').reduce((result, value, index, array) => {
                return result || array.reduce((result, compareValue, compareIndex) => {
                    return !result && index !== compareIndex && Number.isInteger(value / compareValue)
                        ? value / compareValue
                        : result;
                }, 0);
            }, 0);
    ...
    console.log('checksum 2:', day2_2(input));

    const processInput = (input, reduceInitValue, reduceCb) => {
        return input.trim().split('\n').reduce((result, value) => {
            return result += value.trim().split('\t').reduce(reduceCb, reduceInitValue());
        }, 0);
    };
    ...
    const day2_2_refactored = input => processInput(input, () => 0, (result, value, index, array) => {
        return result || array.reduce((result, compareValue, compareIndex) => {
            return !result && index !== compareIndex && Number.isInteger(value / compareValue)
                ? value / compareValue
                : result;
    ...
    console.log('checksum 2 refactored:', day2_2_refactored(input));
