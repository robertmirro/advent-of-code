(input => {
    ...
    const getRootTower = input => {
        const towerMap = {};
        return input.trim().split('\n')
            .map(value => {
                let [ nameWeight, children ] = value.trim().replace(/\(|\)/g, '').replace(/,\s*/g, ',').split('->');
                let [ name, weight ] = nameWeight.trim().split(' ');

                return (towerMap[name] = {
                    name,
                    weight: +weight,
                    children
                });
            ...
            .map(tower => {
                if (tower.children) {
                    tower.children = tower.children.trim().split(',').map(childName => {
                        towerMap[childName].parent = tower;
                        return towerMap[childName];
                    });
                }
                return tower;
            })
            .find(tower => !tower.parent);
    };

    const day7_1 = input => (getRootTower(input) || {}).name;
    console.log('tower 1:', day7_1(input));

    const day7_2 = input => {
        let correctedWeight;
        const getCorrectedWeight = tower => {
            if (!tower.children) tower.weightSum = tower.weight;
            if (tower.children) {
                tower.children.forEach(getCorrectedWeight);
                tower.weightSum = tower.weight +
                    tower.children.reduce((weightSum, tower) => weightSum + (tower.weightSum || tower.weight), 0);

                const weightProp = tower.children[0].weightSum === undefined ? 'weight' : 'weightSum';
                correctedWeight = correctedWeight || tower.children
                    .map(tower => tower[weightProp])
                    .reduce((weightMap, weight, weightIndex, weights) => {
                        weightMap[weight] = (weightMap[weight] || 0) + 1;

                        if (weightIndex === weights.length - 1 && Object.keys(weightMap).length > 1) {
                            const wrongWeight = +Object.keys(weightMap)[Object.values(weightMap).indexOf(1)];
                            const wrongTower = tower.children.find(tower => tower[weightProp] === wrongWeight);
                            const correctTower = tower.children.find(tower => tower[weightProp] !== wrongWeight);

                            weightMap.correctedWeight = wrongTower.weight +
                                (Math.abs(wrongTower[weightProp] - correctTower[weightProp]) * (wrongTower[weightProp] > correctTower[weightProp] ? -1 : 1));
                        }
                        return weightMap;
                    }, {})
                    .correctedWeight;
        ...
        return getCorrectedWeight(getRootTower(input));
    }
    console.log('tower 2:', day7_2(input));
