(input => {
    ...
    const day4_1 = input => {
        return input.trim().split('\n').reduce((result, value) => {
            value = value.trim().replace(/\s\s+/g, ' ').split(' ');
            return result + (value.length === (new Set(value).size) ? 1 : 0);
        }, 0);
    };
    console.log('entrophy 1:', day4_1(input));

    const day4_2 = input => {
        return input.trim().split('\n').reduce((result, value) => {
            value = value.trim().replace(/\s\s+/g, ' ').split(' ').map(value => {
                return value.split('').sort().join('');
            });
            return result + (value.length === (new Set(value).size) ? 1 : 0);
        }, 0);
    };
    console.log('entrophy 2:', day4_2(input));
