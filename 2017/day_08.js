(input => {
    ...
    const shouldUpdateRegister = (operator, registerAmount, value) => {
        if (operator === '>') return registerAmount > value;
        if (operator === '<') return registerAmount < value;
        if (operator === '>=') return registerAmount >= value;
        if (operator === '<=') return registerAmount <= value;
        if (operator === '!=') return registerAmount != value;
    ...
    const updateRegister = (actionType, registerMap, registerName, actionAmount) => {
        if (actionType === 'inc') return registerMap[registerName] += +actionAmount;
        if (actionType === 'dec') return registerMap[registerName] -= +actionAmount;
    };

    const processInput = (input, registerMap) => input.trim().split('\n').map(value => {
        const instruction = value.trim().split(' ');
        registerMap[instruction[0]] = 0;
        return instruction;
    });

    const day8_1 = input => {
        const registerMap = {};
        processInput(input, registerMap)
            .forEach(([ registerName, actionType, actionAmount, conditionType, conditionRegister, conditionOperator, conditionValue  ]) => {
                // const shouldUpdateRegister = eval(`${conditionRegister} ${conditionOperator} ${conditionValue}`); // eval is evil
                if (shouldUpdateRegister(conditionOperator, registerMap[conditionRegister], +conditionValue)) {
                    updateRegister(actionType, registerMap, registerName, actionAmount);
                }
            });
        return Math.max(...Object.values(registerMap));  // object spread required: https://www.chromestatus.com/feature/5657004848709632
    };
    console.log('instructions 1:', day8_1(input));

    const day8_2 = input => {
        const registerMap = {};
        let registerValue, registerMaxValue;

        processInput(input, registerMap)
            .forEach(([ registerName, actionType, actionAmount, conditionType, conditionRegister, conditionOperator, conditionValue  ]) => {
                if (shouldUpdateRegister(conditionOperator, registerMap[conditionRegister], +conditionValue)) {
                    registerValue = updateRegister(actionType, registerMap, registerName, actionAmount);
                    registerMaxValue = registerMaxValue !== undefined && registerValue !== undefined
                        ? Math.max(registerValue, registerMaxValue)
                        : registerValue;
                }
            });
        return registerMaxValue;
    };
    console.log('instructions 2:', day8_2(input));
